...
class ManagementInterface {
    ...
    public function each_archived_before($unix_timestamp, callable $callback) {
        $rows = $this->yield_work_items_cm_before($unix_timestamp, \ISavableWorkItem::CM_ARCHIVED);
        foreach ($rows as $row) {
    ...
    public function each_finished_before($unix_timestamp, callable $callback) {
        $rows = $this->yield_work_items_cm_before($unix_timestamp, \ISavableWorkItem::CM_FINISHED);
        foreach ($rows as $row) {
    ...
    private function yield_work_items_cm_before($unix_timestamp, $change_mask) {
        return \DBIF::get()->yield_work_items_cm_before($unix_timestamp, $change_mask);
    }
}
